...
def fmtline(fieldline, start):
    ...
    if fieldline.count('COMP.') > 0:
        ...
        if fieldline[3][0] == 'X':
            ...
            type = 'Xint'
    elif fieldline[3][0] in ('V',):
        size = int(len(fieldline[3][1:-1]))
        dec = size
        type = 'Xint'
    else:
        dec = 0
...
for line in fd:
    if line[6] == '*':      # drop comment lines
        continue
    newline = line.split()
    if len(newline) == 1 or len(newline) == 0:   # drop blank line
        continue
    newline = newline[1:]
    if 'FILENAME' in newline:
        filename = newline[-1].replace('"','').lower()
        filename = filename.replace('.','')
        output = open('c:/bruske/fd/'+filename+'.fd', 'w')
        code = filename + ' = [\n'
        output.write(code)
    elif newline[0].isdigit() and 'PIC' in newline:
        wrkfd.append(fmtline(newline, start))
        start += wrkfd[-1][-3]
        rec_len += wrkfd[-1][-3]
